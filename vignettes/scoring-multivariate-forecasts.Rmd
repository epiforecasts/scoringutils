...
We can use the function `set_grouping` to specify the grouping. This will return the original data with an additional column `.scoringutils_group_id` that indicates the group each observation belongs to.

```{r}
example_multiv <- example_univ_multi

forecast_unit <- get_forecast_unit(example_multiv)
grouping <- setdiff(forecast_unit, c("location", "location_name"))
example_multiv <- set_grouping(example_multiv, grouping)
```
