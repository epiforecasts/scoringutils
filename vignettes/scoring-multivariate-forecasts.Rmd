...
The grouping variable is set analogously to the forecast unit: by listing all columns that, together, uniquely define a single group - meaning that a unique combination of values in those columns is associated with a single group. You may, however, find it more intuitive to group "across" certain columns. You can do this using `setdiff()` and `get_forecast_unit()`. For example, if you want to make all forecasts for the same location part of the same group, you could do something like `grouping = setdiff(get_forecast_unit(data, "location"))`.

```{r}
forecast_unit <- get_forecast_unit(example_univ_multi)
...
You can notice, that the name of the energy score has changed to `energy_score_multiv` and there is an additional column `.group_id` in the output.
...
As mentioned above, you can set the grouping with the `set_grouping` function. This will return the original data with an additional column `.group_id` that indicates the group each observation belongs to.
