...
The energy score is a multivariate generalisation of the CRPS.
The variogram score (Scheuerer and Hamill, 2015) focuses on the dependence structure by comparing predicted and observed pairwise differences.

You can customise the variogram score parameters using `purrr::partial()`.
For example, to use `p = 1` instead of the default `p = 0.5`:
...
Similarly, we can compute the Variogram Score using the same matrix format.

```{r}
vs_sr1 <- scoringRules::vs_sample(y = obs, dat = fc_sample)
vs_sr2 <- scoringRules::vs_sample(y = obs2, dat = fc_sample2)
...
all.equal(vs_sr, vs_su, tolerance = 1e-6, check.attributes = FALSE)
```

You can customise the variogram score with a weight matrix `w_vs` that controls the importance of each pair of dimensions, and the order parameter `p`.

```{r}
# weight matrix: downweight distant pairs
w_vs <- outer(1:d, 1:d, function(x, y) 0.5^abs(x - y))

vs_su_weighted <- variogram_score_multivariate(
  observed = c(obs, obs2),
  predicted = rbind(fc_sample, fc_sample2),
  mv_group_id = c(rep(1, d), rep(2, d)),
  w_vs = w_vs, p = 1
