...
You can directly pass the output from `check_forecasts()` into the function `score()` (see below). Alternatively, you can access the additional information by using `attributes(x)$info` to access the additionally info from the output of `check_forecasts()`. 
