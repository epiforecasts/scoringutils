...
- Added an internal helper `prepare_forecast_for_scoring()` that consolidates the input preparation steps (cleaning the forecast, determining the forecast unit, validating the metrics and converting to a plain `data.table`) previously duplicated across all `score()` methods (#941).
