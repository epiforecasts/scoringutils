...
This release adds tools for handling missing forecasts (`filter_scores()`, `impute_missing_scores()`), a new `plot_discrimination()` for binary forecasts, and a much faster `get_pairwise_comparisons()`. It also fixes a number of bugs, several of which silently produced wrong scores. Many of these fixes mean that invalid input which previously passed silently now raises an error. Please read the "Breaking changes" section below if you are upgrading.

## Breaking changes

The following changes mean that code which ran without error in scoringutils 2.2.0 may now fail. In all cases the previous behaviour silently produced wrong or corrupted results.
...
- Forecast validation now errors when the same forecast unit has conflicting observed values. Previously, this passed validation for quantile, sample, nominal, ordinal and multivariate sample forecasts, and `score()` silently returned multiple, wrong score rows for a single forecast (#1201).
- `summarise_scores()` now errors when `by` contains a metric column (e.g. `by = c("model", "wis")`). Previously, this silently returned an unsummarised table with duplicate column names (#1204).
- `summarise_scores()` now errors with a clear message when there are no score columns to summarise (e.g. because every metric in `score()` warned and returned nothing). Previously, it returned a data.table with duplicate column names. Metric columns are now matched by exact name rather than by a regex partial match (#1179).
- `get_pairwise_comparisons()` and `add_relative_skill()` now error when the scores have more than one row per forecast unit and comparator. Previously, the duplicated rows were silently included in the comparison (#1221).
- Input validation for categorical (nominal and ordinal) forecasts now errors when `predicted` has more or fewer columns than there are factor levels. This check previously only applied to a single observation, so such input with several observations was accepted and scored meaninglessly (#1200).
- `as_forecast_sample()`, `as_forecast_quantile()` and `as_forecast_multivariate_sample()` now error at validation time when `observed` or `predicted` are not numeric. Previously, the failure only surfaced later inside `score()`, as one warning per metric and an effectively empty scores table (#1211).
...
- `brier_score()` and `logs_binary()`, and therefore `score()` for binary forecasts, now warn when the factor levels of `observed` appear to be in a counterintuitive order (`c("1", "0")` or `c("TRUE", "FALSE")`). Predictions are interpreted as the probability of the second factor level, so with such levels they were silently treated as the probability of "0" or "FALSE", leading to wrong scores (#763).
...
- Fixed `interval_coverage()` erroring on quantile levels generated with `seq()` (e.g. `seq(0.05, 0.95, 0.05)`) because the required quantile levels were matched with an exact floating point comparison. Quantile levels are now rounded to 10 decimal places before matching, consistent with the rest of the package (#1202).
...
- Fixed `rps_ordinal()` and `logs_categorical()` returning wrong scores when called directly with a `predicted_label` that was not in the order of the factor levels: `rps_ordinal()` applied the wrong (forward instead of inverse) permutation when reordering the columns of `predicted`, and `logs_categorical()` ignored `predicted_label` entirely. Scores are now invariant to how the columns of `predicted` are labelled. Forecasts scored via `score()` were unaffected, as the pipeline sorts predictions into level order before calling the metrics (#1200).
- Fixed `get_pit_histogram()` for quantile-based forecasts: it now displays its full warning message and actually falls back to the quantiles present in the forecast when requested quantiles are missing, instead of returning an empty or incorrect result. The rounding warning in `as_forecast_quantile()` now correctly states that quantile levels are rounded to 9 digits (#1211).
- Fixed forecast and scores objects printing spuriously when modified in place with `:=`. Added a `print.scores()` method. This requires data.table 1.17.0 or later, which is now the minimum required version (#935).
...
- Updated the documentation to roxygen2 8.1.0 and fixed two NOTEs from R CMD check on R-devel: a documentation template that listed arguments without a usage section, and a missing import of `stats::ave()` (#1230).
