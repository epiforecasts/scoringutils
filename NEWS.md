...
- `get_duplicate_forecasts()` has a new `type` argument in second position, before `forecast_unit` and `counts`. Code that passed `forecast_unit` or `counts` by position must now name them, e.g. `get_duplicate_forecasts(data, forecast_unit = c("model", "location"))`. A value for `type` that is not a forecast type (such as a column name) raises an error (#888).
...
- Added `get_forecast_type_ids()`, an S3 generic that returns the columns (beyond the forecast unit) that identify a unique row for each forecast type. `get_duplicate_forecasts()` now uses it, and gains a `type` argument (e.g. `get_duplicate_forecasts(data, "quantile")`) for use on plain data.frames (#888).
