...
The following changes mean that code which ran in scoringutils 2.2.0 may now fail. In each case, the previous behaviour produced wrong or corrupted results, often silently.
...
- `get_pairwise_comparisons()` and `add_relative_skill()` now error when the scores contain more than one distinct score per forecast unit and comparator. Previously, such conflicting rows were silently included in the comparison. Exact duplicate rows are still dropped (#1221).
- `logs_categorical()` now errors when `predicted` has more or fewer columns than there are factor levels. Previously, this was only checked for a single observation, so with several observations such input was scored meaninglessly. `score()` is unaffected (#1200).
...
- `assert_forecast()` for nominal and ordinal forecasts now returns `invisible(NULL)`, as documented and as for all other forecast types. Previously, it visibly returned the forecast object (#1195).
- scoringutils now requires data.table 1.17.0 or later (previously 1.16.0) (#935).

## Deprecations

- Calling `get_duplicate_forecasts()` on a plain data.frame without specifying `type` is deprecated and now warns. It falls back to guessing type-specific columns from their names (#888).
...
- Fixed `as_forecast_quantile()` for sample-based forecasts producing silently wrong quantiles, or erroring, when `probs` was not symmetric around 0.5 (e.g. `probs = 0.4` or `probs = c(0.1, 0.2)`). Quantiles are now computed at exactly the requested `probs` (deduplicated), and out-of-range `probs` produce a clear error (#1196).
...
- Fixed forecast and scores objects printing spuriously when modified in place with `:=`, and added a `print.scores()` method (#935).
