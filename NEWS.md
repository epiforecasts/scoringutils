...
- Forecast validation now errors when the same forecast unit has conflicting observed values. Previously, this passed validation for quantile, sample, nominal, ordinal and multivariate sample forecasts, and `score()` silently returned multiple, wrong score rows for a single forecast. This usually means that the forecast unit is missing a column; check it with `get_forecast_unit()` or set it with `set_forecast_unit()` (#1201).
...
- `logs_categorical()` now errors when `predicted` has more or fewer columns than there are factor levels. Previously, this was only checked for a single observation, so with several observations such input was scored meaninglessly. `score()` is unaffected. `rps_ordinal()`, which already errored on such input, now gives a clearer error (#1200).
