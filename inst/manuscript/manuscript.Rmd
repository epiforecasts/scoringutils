...
Pairwise comparisons should usually be made based on raw, unsummarised scores (i.e. `add_pairwise_comparison()` should be called before `summarise_scores()` (see Section XXX)). Summarising scores, for example by computing an average across several dimensions, can change the set of overlapping forecasts between two models and distort relative skill scores.
