...
The following section gives an overview of the main functionality of \pkg{scoringutils}. Everything will be illustrated using the example data shipped with the package, which contains a subset of forecasts submitted to the European COVID-19 Forecast Hub \citep{europeancovid-19forecasthubEuropeanCovid19Forecast2021}.
An overview of all existing functions can be seen on \url{https://epiforecasts.io/scoringutils}. 

## Basic workflows

\pkg{scoringutils} supports two basic workflows. Most users will interact with \pkg{scoringutils} through a framework based on a `data.frame`-like structure (see Figure \ref{fig:workflow-scoringutils}A). Within this framework they can complete a variety of different tasks such as validating and diagnosing inputs, transforming inputs, scoring, aggregating scores, and visualisation. 
...
```{r workflow-scoringutils, echo = FALSE, fig.pos = "!h", out.width="100%", fig.cap= "Illustration of potential workflows for evaluation forecasts with scoringutils. A: Workflow for working with forecasts in a data.frame-based format. B: Using scoring rules directly with vectors/matrices as inputs.", fig.show="hold"}
include_graphics("output/workflow.png")
```

## Input validation and types of forecasts

An evaluation workflow with \pkg{scoringutils} usually starts with a data.frame (or similar) containing both the predictions and the observed values. The input data can be processed and validated in the following way: 
...
The function \fct{as\_forecast} transforms the input into a forecast object. A forecast object is a `data.table` that has passed some input validations and can have additional attributes. It behaves like a `data.table`, but has additional dedicated methods e.g. for input validation, scoring and printing. There are currently four different kinds of forecast objects with classes `forecast_point`, `forecast_binary`, `forecast_quantile` and `forecast_sample`. These classes correspond to different forecast types: "binary", "point", "quantile" and "sample". 
...
Each forecast type comes with a specific input format. All forecasts require columns `observed` and `predicted`, but depending on the forecast type, additional requirements may be in place. Table \ref{fig:input-score} shows the expected input format for each forecast type. When converting a `data.frame` (or similar) into a forecast object, the type of the forecast will be determined based on the names and the type of the columns present in the input data. \fct{as\_forecast} will then construct an object of the appropriate `forecast_*` class and validate it using the function \fct{validate\_forecast} (see Figure \ref{fig:flowchart-validation}). Users can call the function \fct{validate\_forecast} directly to validate a forecast object again.

<!-- Input formats for the scoring rules that can be called directly usually follow the same convention, with inputs expected to be vectors or matrices.  -->
...
Probabilistic forecasts are usually composed of several values. A quantile-based forecast, for example, is composed of several quantiles, and a sample-based forecast of multiple samples. Therefore, several rows in the input data can belong together and jointly form a single forecast. For example, a single forecast in a quantile-based format (and the corresponding observation) could be represented by five rows in a data.frame with values `c(0.05, 0.25, 0.5, 0.75, 0.95)` in the column `quantile_level`, `c(1, 4, 5, 6, 9)` in the column `predicted` and the value `7` in the column `observed`. 

Rows are automatically grouped together based on the values in all other columns present in the data (excluding required columns and values computed by \pkg{scoringutils}). The combination of values in those columns form the unit of a single forecast (or "forecast unit") and should uniquely identify a single forecast. For example, consider forecasts made by different models in various locations at different time points and for different targets. A single forecast could then be uniquely described by the values in the columns `model`, `location`, `date`, and `target`, and the forecast unit would be `forecast_unit = c("model", "location", "date", "target")`. 
As the forecast unit is internally determined based on all existing columns, no column must be present that is unrelated to the forecast unit. As a very simplistic example, consider an additional row, "even", that is one if the row number is even and zero otherwise. The existence of this column would change results, as \pkg{scoringutils} assumes it was relevant to grouping the forecasts. You can get the current forecast unit by calling \fct{get\_forecast\_unit} on the data or by simply printing the object if it is of class `forecast_*`.

To avoid issues, we recommend using the function \fct{set\_forecast\_unit} to determine the forecast unit manually. The function simply drops unneeded columns, while retaining required columns and those added by \pkg{scoringutils}. Usually, \fct{set\_forecast\_unit} would be called before \fct{as\_forecast}, but the order can also be swapped. If the input has been validated before (i.e. if an object of class `forecast_*` is passed to \fct{set\_forecast\_unit}), validations will be rerun again to ensure dropping columns has not caused issues.
...
<!-- Next to paragraphs could be more concise -->
\pkg{scoringutils} requires that there be no duplications in the data, either with respect to whole forecasts or to parts of a forecast. Only one forecast per forecast unit is allowed. For instance, continuing the example above, there must not be two forecasts for the same `model`, `location`, `date`, and `target`. 
For quantile- or sample-based forecasts, there must not be two values with the same `quantile_level` or `sample_id` for a single forecast unit. Predicted values with the same forecast unit, but different quantile levels or sample id will simply interpreted as components of the same forecast. 

The function \fct{get\_duplicate\_forecasts} may be helpful in case \fct{validate\_forecast} returns an error message about duplicate forecasts. Duplicate forecasts can be a sign of an ill-defined forecast unit. For example, if we dropped the column `target_type` in the example data, we would obtain an error about duplicate forecasts. Even though the predicted values differ, there are now two different forecasts for the same forecast unit, since \pkg{scoringutils} cannot distinguish between cases and deaths any more. \fct{get\_duplicate\_forecasts} finds duplicates and returns them for the user to inspect. To remedy the issue, users need to add additional columns that uniquely identify a single forecast.
...
quantile_forecast <- as_forecast(example_quantile) 
class(quantile_forecast)
```
...
\fct{get\_forecast\_counts} returns forecast counts, which helps obtain an overview of missing forecasts. This can impact the evaluation, if missingness correlates with performance. Users can specify the level of summary through the `by` argument. For example, to see how many forecasts there are per model and target_type, we can run
...
The forecasts and observed values themselves can be visualised using the \fct{plot\_predictions} function. The function works on a validated object of class `forecast_*`.
...
Explain the work flow when as_forecast has been used (i.e. default metrics and validation for that class).
...
The above definition of probabilistic calibration also implies that the empirical coverage of the central prediction intervals formed by the predictive quantiles should be equal to the nominal interval coverage. For example, the central 50\% prediction intervals of all forecasts should contain around 50\% of the observed values, the 90\% central intervals should contain around 90\% of observations etc. Forecasts that are too narrow and do not cover the required proportion of observations are called overconfident or underdispersed, while predictive distributions that are too wide are often called underconfident, overdispersed or conservative. 

Users can obtain coverage values for quantile-based predictions in two different ways. The first is to call the functions \fct{interval_coverage_quantile} and \fct{quantile_coverage_quantile} directly as scoring rules within `score()` to obtain coverage values for specific quantile-levels or central prediction intervals. A more comprehensive way is by using the function \fct{add\_coverage} directly on the raw forecasts. It adds additional columns with the width of the central prediction interval corresponding to a given quantile level, quantile coverage, interval coverage, quantile coverage deviation and interval coverage deviation. Deviation here means the difference between nominal and empirical coverage. Coverage for a single quantile or interval is only ever `TRUE` or `FALSE`. Coverage values are therefore only meaningful when summarised over many forecasts. This can be done by calling `aggregate()`, as \pkg{scoringutils} provides a method for class `forecast_quantile` (which behaves the same as the \fct{aggregate} method for class `score` discussed above). 
