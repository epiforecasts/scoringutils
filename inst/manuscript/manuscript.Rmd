...
Various \proglang{R} \citep{R} packages implement a large number of scoring rules, plots and other metrics that are useful in assessing the quality of a forecast. Some notable examples are \pkg{scoringRules} \citep{scoringRules}, \pkg{Metrics} \citep{Metrics}, \pkg{MLmetrics} \citep{MLmetrics}, \pkg{yardstick} \citep{yardstick}, \pkg{scoring} \citep{scoring}, \pkg{verification} \citep{verification}, \pkg{surveillance} \citep{surveillance}, \pkg{predtools} \citep{predtools}, \pkg{probably} \citep{probably}, \pkg{topmodels} \citep{topmodels}, \pkg{SpecsVerification} \ citep{SpecsVerification}, \pkg{tscount} \citep{tscount}, \pkg{GLMMadaptive} \citep{GLMMadaptive},and \pkg{cvGEE} \citep{cvGEE}. These cover a wide range of (in some instances overlapping) functionality. From a user perspective, however, several challenges remain that \pkg{scoringutils} aims to address. 

One potential challenge, particularly to inexperienced users, is the large number of existing packages itself. This is in parts an issue of discoverability (i.e. users not being aware about what functions exist where) and to some extent an issue of usability (i.e. users having to navigate different formats and packages). \pkg{scoringutils} aims to bridge the gap between existing packages and to make it easier to work with different packages in the ecosystem a coherent way. The package provides a clear structure and interface for applying a wide variety of metrics to different types of forecasts. Users can easily apply scoring functions from different packages or supply their own scoring rules and metrics. 
To alleviate the discoverability issue, \pkg{scoringutils} provides extensive vignettes and case studies that illustrate how functions from different packages can be used together. To address the usability issue, it provides helper functions to convert to and from the formats used in different packages, where those are not immediately compatible, making it easy to use different packages in a single workflow. In the future, we also aim to provide interfaces to common modelling packages such as \pkg{odin} or XXX, integrating modelling and evaluation more closely. In general, \pkg{scoringutils} aims to be easily extendable, providing generics and methods that users can expand on, functionality to apply custom scores, and a large number of general purpose diagnostic functions to verify inputs. 

Another challenge in forecast evaluation is scoring and comparing forecasts from multiple models, across multiple dimensions such as time and space and different targets. Existing such packages such as for example \pkg{scoringRules}, \pkg{Metrics}, \pkg{MLmetrics}, \pkg{verification}, \pkg{SpecsVerification} provide an extensive collection of scoring rules and metrics. These functions usually operate on vectors and matrices, meaning that applying them to multiple forecasts across several dimensions can be cumbersome. Some packages like \pkg{scoring} operate on a data.frame and use a formula interface. However, \pkg{scoring} only exports a few function and does not provide a general interface that would allow applying arbitrary scoring rules. 
\pkg{yardstick}, which builds on the \pkg{tidymodels} \citep{tidymodels} framework, addresses that issue of scoring forecasts across various dimensions and allows users to apply arbitrary scoring functions to a data.frame. The package is very general and easily extendable and it that sense quite similar to what \pkg{scoringutils} aims to be. Just like \pkg{scoringutils}, \pkg{yardstick} allows users to score forecasts using arbitrary scoring functions and to aggregate and compare results. However, \pkg{yardstick} is primarily focused on point forecasts and classification tasks. It currently lacks general support for probabilistic forecasts (that come e.g. in the form of predictive samples or quantiles, or other forms), which are one of the main focus points of \pkg{scoringutils}. In particular, \pkg{scoringutils} is the first package to provide extensive support for probabilistic forecasts in the form of predictive quantiles, a format that is currently used by several infectious disease Forecast Hubs \citep{reichCollaborativeMultiyearMultimodel2019, cramerCOVID19ForecastHub2020, europeancovid-19forecasthubEuropeanCovid19Forecast2021, bracherNationalSubnationalShortterm2021}. Additional functions to handle missing forecasts or transform forecasts \citep{ScoringEpidemiologicalForecasts} are also provided.

In contrast to some packages such as \pkg{tscount}, \pkg{topmodels}, \pkg{GLMMadaptive} or \pkg{cvGEE}, \pkg{scoringutils} aims to provide tools that are agnostic to how the forecasts were generated. The aforementioned packages provide useful scoring metrics and plots. However, those are only accessible if the forecasts were generated in a particular way (i.e. \pkg{tscount} requires an object of class `tsglm`, \pkg{topmodels} requires an object of class `lm`, `glm`, `crch` or `disttree`, \pkg{GLMMadaptive} requires an object of class `MixMod` and \pkg{cvGEE} requires an object of class `geeglm`). In contrast, \pkg{scoringutils} aims to allow users to score forecasts in various formats, irrespective of how these forecasts were created. The package currently does not handle all possible kinds of prediction formats supported by other packages. For example, it does not support multiclass predictions, or scoring forecasts that are specified as a closed-form distribution. However, we plan to add support for these to the package in the near future, as well as support for scoring multivariate forecasts that specify a joint distribution across targets.

Lastly, a challenge for inexperienced users is to know which metric to use. \pkg{scoringutils} facilitates the process by asking users only for a data set in the right format corresponding to the prediction task. Sensible defaults then allow the user to get started easily. In addition, the package provides extensive function documentation and explanations of the default metrics, as well as vignettes and case studies that illustrate how to use the package and interpret results. 
...
## Aim of the package:
- simplify the process of forecast evaluation
- serve as a general purpose framework that users can extend and expand on
- provide helper functions to convert data formats and transform the data. 
...
The package provides several functions, classes, and generics. 
...
- maybe other classes for plotting

### score() and summarise scores()

\fct{score()} is the workhorse of the package. It takes a data.frame as input as well as a list of scoring functions and applies those scoring functions to the input, returning a data.frame with the scores. 

\fct{summarise\_scores()} allows users to aggregate scores across different dimensions.

### Diagnostic functions to obtain information about the data

Functions that start with `get_` provide some information on the data and the input. 
Examples are `get_forecast_counts`, `get_forecast_unit`

### Data handling and data conversion
...
### Scoring rules
Functions with various metrics and scoring rules. 
Naming: name of the metric + \_ + name of the expected input format (see below). Return a vector. 


### Plotting functions


\fct{score()} is a generic. The default method runs \fct{validate\_forecasts} on the input and calls \fct{score()} again afterwards (see Figure \ref{fig:illustration-score}). `validate_forecasts()` calls a series of check functions to determine the type of the forecast and the input format. Based on those checks, it constructs an object of class `scoringutils_binary`, `scoringutils_point`, `scoringutils_sample` or `scoringutils_quantile` (see below) and validates that object. Upon calling `score()` again, the function dispatches to the appropriate method based on the class of the input. The dedicated methods then apply the scoring functions to the data. 

Output of `score()`: 


```{r illustration-score, echo = FALSE, fig.pos = "!h", out.width="70%", fig.cap= "Illustration of the workflow for \fct{score()}.", fig.show="hold"}
include_graphics("output/illustration-score.png")
```


# Forecast types and input formats

`scoringutils` offers two ways to score forecasts: users can either use the function `score()` which applies scoring rules to a data.frame (or similar) of forecasts. Or they can call the individual scoring rules directly on a set of vectors and matrices. The format of the expected input depends on the type of the forecast to evaluated. 
Forecast may come in many different ways, depending on the target and how the forecaster chooses to represent their prediction. To distinguish between different prediction tasks and possible ways of representing forecasts, `scoringutils` uses the term "forecast type". This is more a convenient classification than a formal definition. At the moment, `scoringutils` distinguishes four different forecast types: "binary", "point", "quantile" and "sample" forecasts (support for more forecast types is planned for the future).

"Binary" denotes a probability forecast for a binary (yes/no) outcome variable. This is sometimes also called "soft binary classification". "Point" denotes a forecast for a continuous or discrete outcome variable that is represented by a single number. "Quantile" is used to denote a probabilistic forecast for a continuous or discrete outcome variable, with the forecast distribution represented by a set of predictive quantiles. "Sample" is used to denote a probabilistic forecast for a continuous or discrete outcome variable, with the forecast represented by a finite set of samples drawn from the predictive distribution.

## Input formats for score

When evaluating forecasts as a data.frame, `scoringutils` distinguishes between the different forecast types based on the names and the type of the columns present in the input. Table \ref{fig:input-score} shows the expected input format for each forecast type. Forecast types that are planned, but not currently supported, are greyed out. 

```{r input-score, echo = FALSE, fig.pos = "!h", out.width="70%", fig.cap= "Table with different input formats.", fig.show="hold"}
include_graphics("output/input-score.png")
```


## Input formats for scoring rules

Input formats for the scoring rules that can be called directly follow the convention used for `score()`, with inputs expected to be vectors or matrices.
...
Depending on the format of the forecast, a `data.frame` (or similar) is required for most functions with column names as shown in Table \ref{tab:column-requirements}. Point forecasts follow the same format as quantile-based format (and can be mixed with quantile-based forecasts), but are defined by the fact that they have an `NA` value in the column `"quantile"`. 
...
Using the `metrics` argument, we can control which metrics to compute. The above produces one score for every forecast. However, we usually like to summarise scores to learn about average performance across certain categories. This can be done using the function \fct{summarise\_scores}, which returns one summarised score per category (column name) specified in the argument \code{by}. To return, for example, one score per model and forecast target for two-week-ahead forecasts, we can run the following: 
...
Using the function \fct{plot\_pairwise\_comparison} we can visualise mean score ratios for the WIS between all models. 

The output of the following code is shown in Figure \ref{fig:pairwise-plot}. 
