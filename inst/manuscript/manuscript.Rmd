...
<!-- While specific metrics may differ depending on the forecast type or format, the general forecasting paradigm \cite{gneitingProbabilisticForecastsCalibration2007} that guides the evaluation process is the same. -->
...
<!-- \begin{tabular}{lll} -->
<!-- \hline -->
<!-- Forecast type                                                 & Target type                                                                                 & Representation of the predictive distribution                                                           \\ \hline -->
<!-- \multicolumn{1}{|l|}{Point forecast}                          & \multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}continuous\\ discrete\\ binary\end{tabular}} & one single number for the predicted outcome                                                             \\ \hline -->
<!-- \multicolumn{1}{|l|}{\multirow{2}{*}{probabilistic forecast}} & \begin{tabular}[c]{@{}l@{}}continuous\\ discrete\end{tabular}                               & \begin{tabular}[c]{@{}l@{}}predictive samples, \\ closed analytical form, \\ or quantiles,\end{tabular} \\ \cline{2-2} -->
<!-- \multicolumn{1}{|l|}{}                                        & binary                                                                                      & binary probabilities                                                                                    \\ \hline -->
<!-- \end{tabular} -->
<!-- \end{table} -->
...
\begin{table}[]
\centering
\caption{Forecast and forecast target types. Forecasts can be distinguished by whether they are probabilistic in nature, or a point forecast only. Depending on the type of the target (discrete, continuous or binary) different representation of the predictive distribution are possible.}
\label{tab:forecast-types}
\begin{tabular}{@{}lll@{}}
\toprule
...
data[, 1:8] |>
  ...
  kableExtra::column_spec(3, width = "2.7cm") |>
...
```{r calibration-plots, echo = FALSE,  fig.pos = "!h", out.extra = "", fig.cap= "Calibration plots for different forecast distributions.", cache = FALSE}
...
For sample-based forecasts, \pkg{scoringutils} calculates dispersion (the inverse of sharpness) as the normalised median absolute deviation about the median (MAD), following \cite{funkAssessingPerformanceRealtime2019} (for details see Table \ref{tab:score-overview}). For quantile forecasts, we instead report the dispersion component of the weighted interval score (see details in section \ref{wis} and \ref{score-table-detailed}) which corresponds to a weighted average of the individual interval widths. 
...
<!-- Add part about how they deal differently with uncertainty. I.e. Log score prefers more uncertain forecasts -->
...
```{r score-convergence, echo = FALSE, fig.cap="Convergence of different scores. MORE DETAIL"}
...
As the CRPS is a generalisation of the absolute value, overall scores usually depend on the order of magnitude of the quantity we try to forecast (as the variance of the data-generating distribution usually increases with the mean). This makes it harder to compare forecasts for very different targets, or assess average performance if the quantity of interest varies substantially over time. Average scores tend to be dominated by forecasts for targets with high absolute numbers. This may be desirable,if we care most about forecasts in situations where numbers are high, but usually it is not. LogS and DSS are more robust against this effect. Another way to address this issue by using pairwise comparisons will be introduced later. 
<!-- Maybe have a plot for this? -->
...
```{r score-heatmap, fig.pos = "!h", fig.width = 8, fig.cap = "CAPTION"}
...
```{r wis-components, fig.pos = "!h", fig.width = 8, fig.cap = "CAPTION"}
score(example_quantile) |>
  summarise_scores(by = c("model", "target_type")) |>
...
```{r pit-plots, fig.pos = "!h", fig.cap="CAPTION", fig.width = 8, fig.height=4}
example_continuous |>
