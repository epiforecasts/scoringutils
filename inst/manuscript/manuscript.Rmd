...
The \pkg{scoringutils} package implements a variety of metrics which focus on sharpness or calibration alone, as well as several so-called proper scoring rules, which combine both aspects into a single number. The former can be helpful to learn about specific model aspects and improve them. The latter are especially useful to assess and rank predictive performance of a forecaster. Table \ref{tab:table-summary-scores} shows an overview of the metrics currently implemented in \pkg{scoringutils} and when they are applicable. Table \ref{tab:score-table} in the Appendix gives a mathematical definition and more thorough explanation of all the metrics. 
...
#### Sensitivity to distance: local and global scores
...
### Proper scoring rules for quantile-based forecasts (WIS and BS)
...
# Evaluating forecasts using scoringutils - an case study {#evalutation-example}

Generally there are two ways in which scores can be used: as lower level functions or as part of a data.frame-based workflow. 
...
```{r,tidy=TRUE, tidy.opts=list(width.cutoff=I(70))}
