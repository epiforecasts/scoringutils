...
An overview of all existing functions can be seen in the on \url{https://epiforecasts.io/scoringutils}. 

## Basic workflow

\pkg{scoringutils} supports two basic workflows (see Figure \ref{fig:workflow}). One option is for users to score their forecasts directly using the scoring rules exported by \pkg{scoringutils} in a format based on vectors and matrices. Those functions that can be called directly on a set of vectors and matrices and return a single vector of scores will consistently be called "scoring rules" in the following. Most users, however, will interact with \pkg{scoringutils} through a more convenient framework based on a `data.frame`-like structure (internally, \pkg{data.table} is used). 
...
The example data used througout comes from X. It conforms to all inputs and provides a reference point for users. 

Here is the output of validating the example data. 

```{r example-data, echo = FALSE, fig.pos = "!h", out.width="100%", fig.cap= "Output of validating the example data.", fig.show="hold"}
example_quantile |>
  validate()
```
...
Functions that provide this kind of additional information usually start with `get_`. 
...
```{r get-forecast-counts, echo = FALSE}
forecast_counts <- available_forecasts(example_quantile, by = c("model", "target_type"))
```

This returns an object of class `forecast_counts`. We can visualise the results by calling \fct{plot} on the object. 

```{r plot-forecast-counts, echo = FALSE}
plot(forecast_counts, xvar = "target_type")
```

## Visualising the data

This is currently still a bit terrible.  

## Scoring forecasts

As mentioned above, users can either call different scoring rules directly on vectors and matrices or use the function `score()` on a data.frame (or similar) to apply multiple scoring rules at once. 

### score()

The workhorse of the package is the function `score()`. It takes as input either an object of class `forecast_*` or a `data.frame` (or similar) with forecasts, as well as a list of functions (the scoring rules). The function then applies those scoring rules to the input. Additional arguments can be passed down to the scoring rules via `...`. \fct{score} is a generic function that dispatches to different methods depending on the class of the input.
...
Input formats and return values are shown in more detail in Figure \ref{fig:input-score}. 

```{r inputs-scoring-rules, echo = FALSE, fig.pos = "!h", out.width="100%", fig.cap= "Flowchart for calling `score()`", fig.show="hold"}
...
## The unit of a single forecast {short-title="The unit of a single forecast" #forecastunit}
