...
% All forecast types
\multirow{3}{*}{All forecast types} & & & \texttt{observed}  & \\
                                    & & & \texttt{predicted} & \\
                                    & & & \texttt{model}    & \\
\midrule
...
\multirow{2}{*}{Classification} & \multirow{2}{*}{Binary} & Soft classification & \texttt{observed} & factor with 2 levels \\ 
                                &        & (prediction is probability)  & \texttt{predicted} & numeric [0,1]       \\
\midrule

% Point forecasts
\multirow{2}{*}{Point forecasts} & \multicolumn{2}{l}{\multirow{2}{*}{\makecell[cl]{Discrete, \\Continuous}}} & \texttt{observed}  & numeric                \\
                                 & & & \texttt{predicted} & numeric                \\
\midrule

% Probabilistic forecast
\multirow{6}{*}{\makecell[cl]{Probabilistic\\forecast}} & \multirow{6}{*}{\makecell[cl]{Discrete, \\Continuous}} & \multirow{3}{*}{Sample format} & \texttt{observed} & numeric                \\
                                                         &                      &               & \texttt{predicted} & numeric                \\
...
```{r input-score, echo = FALSE, fig.pos = "!h", out.width="100%", fig.cap= "Table with expected input formats for \\fct{score}. Regardless of the forecast type, the \\code{data.frame} (or similar) must have columns called \\code{observed} and \\code{predicted} and should have a column called \\code{model}. For binary forecasts, the column \\code{observed} must be of type factor with two levels and the column \\code{predicted} must be a numeric betwen 0 and 1. For all other forecast types, both \\code{observed} and \\code{predicted} must be of type numeric. Forecasts in a sample-based format require an additional numeric column \\code{sample\\_id} and forecasts in a quantile-based format require an additional numeric column \\code{quantile\\_level} with values between 0 and 1.", fig.show="hold"}
include_graphics("output/input-score.png")
```

The expected input is a data.frame (or similar) containing both the predictions and the observed values. At the minimum, the data.frame must have columns called `observed` and `predicted` and should also have a column called `model` denoting the name of the model/forecaster that generated the forecast. Additional requirements depend on the forecast type. Table \ref{fig:input-score} shows the expected input format for each forecast type. 
