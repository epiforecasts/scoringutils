...
test_that("Input checking for nominal forecasts works", {
  # everything correct
  expect_no_condition(
    scoringutils:::assert_input_nominal(observed, predicted, predicted_label)
  )

  # observed and predicted_label have different level lengths
  observed_wrong <- factor(observed, levels = c("one", "two"))
  expect_error(
    scoringutils:::assert_input_nominal(observed_wrong, predicted, predicted_label),
  ...
  expect_error(
    scoringutils:::assert_input_nominal(observed, predicted, predicted_label_wrong),
    "Assertion on 'predicted_label' failed: Contains missing values"
  )

  # 6 observations, but only 3 forecasts
  expect_error(
    scoringutils:::assert_input_nominal(c(observed, observed), predicted, predicted_label),
    "Assertion on 'predicted' failed: Must have exactly 6 rows, but has 3 rows."
  )

  # observed value or predicted_label is not factor
  expect_error(
    scoringutils:::assert_input_nominal(as.numeric(observed), predicted, predicted_label),
    "Assertion on 'observed' failed: Must be of type 'factor', not 'double'."
  )
  expect_error(
    scoringutils:::assert_input_nominal(observed, predicted, as.character(predicted_label)),
    "Assertion on 'predicted_label' failed: Must be of type 'factor', not 'character'."
  )

  # a single observation
  expect_no_condition(
    scoringutils:::assert_input_nominal(observed[1], predicted[1, ], predicted_label),
  )
  expect_no_condition(
    scoringutils:::assert_input_nominal(observed[1], as.numeric(predicted[1, ]), predicted_label),
  )

  # wrong dimensions
  expect_error(
    scoringutils:::assert_input_nominal(observed[1], predicted, predicted_label),
  ...
  expect_no_condition(
    scoringutils:::assert_input_nominal(observed2, predicted, predicted_label)
  )
  ...
  predicted2[2, 2] <- NA
  expect_error(
    scoringutils:::assert_input_nominal(observed, predicted2, predicted_label),
