...
test_that("pit function works for continuous integer and quantile data", {
  pit_quantile <- suppressMessages(as_forecast_quantile(example_quantile)) %>%
    get_pit(by = "model")
  pit_continuous <- suppressMessages(as_forecast_sample(example_sample_continuous)) %>%
    get_pit(by = c("model", "target_type"))
  pit_integer <- suppressMessages(as_forecast_sample(example_sample_discrete)) %>%
