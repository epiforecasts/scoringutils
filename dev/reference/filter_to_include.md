...
that restricts the kept target combinations to those covered by every
value listed in `include`. With a single value this keeps only that
value's targets; with several values, the intersection of their target
sets is kept.

To use a proportion-based threshold over all compare values instead, use
[`filter_to_intersection()`](https://epiforecasts.io/scoringutils/dev/reference/filter_to_intersection.md).

## Usage

``` r
filter_to_include(include)
```
...
- include:
  ...
  Character vector of length one or more. Values from the `compare`
  column whose target sets should be intersected.
